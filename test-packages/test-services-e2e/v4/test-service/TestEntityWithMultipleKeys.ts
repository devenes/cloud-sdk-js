...
export class TestEntityWithMultipleKeys<T extends DeSerializers = DefaultDeSerializers> extends Entity implements TestEntityWithMultipleKeysType<T> {
  ...
  boundActionWithoutArguments<DeSerializersT extends DeSerializers = DefaultDeSerializers>(): BoundActionRequestBuilder<DeSerializersT, any, undefined | null> {
    ...
    return new BoundActionRequestBuilder(
      '/odata/test-service', 'TestEntityWithMultipleKeys', entityQueryString, 'TestService', 'boundActionWithoutArguments', (data) => data, params, deSerializers
    );
  }
  ...
  deleteEntity<DeSerializersT extends DeSerializers = DefaultDeSerializers>(): BoundActionRequestBuilder<DeSerializersT, any, undefined | null> {
    ...
    const entityQueryString = TestEntityWithMultipleKeys._keys.map(key => key + '=' + this[camelCase(key) as keyof TestEntityWithMultipleKeys]).join(',');
    return new BoundActionRequestBuilder(
      '/odata/test-service', 'TestEntityWithMultipleKeys', entityQueryString, 'TestService', 'deleteEntity', (data) => data, params, deSerializers
    );
  }
}
