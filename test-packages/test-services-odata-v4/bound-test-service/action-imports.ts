 ...
 */
export function createTestEntityById<DeSerializersT extends DeSerializers = DefaultDeSerializers>(parameters: CreateTestEntityByIdParameters<DeSerializersT>, deSerializers: DeSerializersT = defaultDeSerializers as any): ActionImportRequestBuilder<DeSerializersT, CreateTestEntityByIdParameters<DeSerializersT>, TestEntity | null> {
  const params = {
    id: new ActionImportParameter('id', 'Edm.Int32', parameters.id)
  }

  return new ActionImportRequestBuilder('/odata/test-service', 'createTestEntityById', (data) => transformReturnValueForEntity(data, boundTestService(deSerializers).testEntityApi), params, deSerializers);
}
