...
class TestEntityWithMultipleKeys extends odata_v4_1.Entity {
    ...
    deleteEntity() {
        const params = {};
        const deSerializers = odata_v4_1.defaultDeSerializers;
        const entityQueryString = TestEntityWithMultipleKeys._keys.map(key => key + '=' + this[(0, util_1.camelCase)(key)]).join(',');
        return new odata_v4_1.BoundActionRequestBuilder('/odata/test-service', 'TestEntityWithMultipleKeys', entityQueryString, 'BoundTestService', 'deleteEntity', (data) => data, params, deSerializers);
    }
    boundActionWithoutArguments() {
        const params = {};
        const deSerializers = odata_v4_1.defaultDeSerializers;
        const entityQueryString = TestEntityWithMultipleKeys._keys.map(key => key + '=' + this[(0, util_1.camelCase)(key)]).join(',');
        return new odata_v4_1.BoundActionRequestBuilder('/odata/test-service', 'TestEntityWithMultipleKeys', entityQueryString, 'BoundTestService', 'boundActionWithoutArguments', (data) => data, params, deSerializers);
    }
}
