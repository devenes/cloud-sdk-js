...
class TestEntity50Prop extends odata_v4_1.Entity {
    ...
    deleteEntity() {
        const params = {};
        const deSerializers = odata_v4_1.defaultDeSerializers;
        const entityQueryString = TestEntity50Prop._keys.map(key => key + '=' + this[(0, util_1.camelCase)(key)]).join(',');
        return new odata_v4_1.BoundActionRequestBuilder('/odata/test-service', 'TestEntity50Prop', entityQueryString, 'BoundTestService', 'deleteEntity', (data) => data, params, deSerializers);
    }
    boundActionWithoutArguments() {
        const params = {};
        const deSerializers = odata_v4_1.defaultDeSerializers;
        const entityQueryString = TestEntity50Prop._keys.map(key => key + '=' + this[(0, util_1.camelCase)(key)]).join(',');
        return new odata_v4_1.BoundActionRequestBuilder('/odata/test-service', 'TestEntity50Prop', entityQueryString, 'BoundTestService', 'boundActionWithoutArguments', (data) => data, params, deSerializers);
    }
}
